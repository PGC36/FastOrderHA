...
Por defecto, la observabilidad multi-host asume:

- `api-gateway`, `menu-service`, `order-service`, `inventory-service`, `kitchen-service` en `PC2`
- `delivery-service`, `notification-service` en `PC3`
- `RabbitMQ` en `PC2`

Si cambian esa distribucion, actualicen en `multi-host.env`:
